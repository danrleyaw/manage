...
const App: React.FC = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser(res.user);
      await loadAdminGames(res.user.id);
      setView('dashboard');
      ...
      <div className="w-full max-w-[360px] relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="space-y-8">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <button 
              type="submit" 
              className="w-full bg-slate-900 hover:bg-blue-600 text-white py-4.5 rounded-2xl font-heading italic uppercase tracking-widest active:scale-[0.98] transition-all text-sm flex items-center justify-center gap-3 group shadow-xl"
            >
